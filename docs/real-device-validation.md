...
- 已验证代码：`610dd529c1ed45eaa831afd802f2a8ebe2f94532`，Pi 5 / Debian 13 /
  aarch64 / Python 3.13.5；隔离目录应用同一代码后验证。
- 树莓派完整回归 321 passed，1 条既有第三方弃用警告，21.77 秒，库覆盖率 90.21%。
  Windows 319 passed、2 skipped，库覆盖率 90.13%。覆盖率统计不包含 scripts 目录。
- pip check、Ruff、部署预检、API 配置检查和本机 API 启停检查通过。
- 在树莓派 Wayland 桌面终端实际运行 `scripts/run_safe_demos.py`，六个阶段退出码均为 0，
  `summary.json` 的 `complete`、`ok` 均为 true；日志和演示数据保存在仓库外的独立目录。
- 此轮未打开摄像头、未驱动 GPIO；传感器和执行器仍是模拟，不把演示成功当作外设验证。
