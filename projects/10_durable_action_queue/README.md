...
```bash
python projects/10_durable_action_queue/main.py run-demo --json --fail-on-error
```

`--json` 关闭逐项文字输出，成功完成调度后仅输出一行 JSON 报告，包含处理数量及完成、失败、
死信、延后和租约中动作 ID。`--fail-on-error` 在本次有动作执行失败时返回退出码 1，仍输出
完整报告，不中断后续动作，也不撤回已成功动作或失败重试记录。没有实际执行失败时返回 0；
历史死信不影响本次退出码，检查历史死信请使用 `status --fail-on-dead`。
两个选项可以单独使用；省略时保留原有输出和退出行为。参数或存储错误仍按现有错误路径处理，
不能仅根据存在 JSON 输出判断成功，脚本应同时检查退出码。这仍是模拟调度，不控制真实设备。
